...
/* ===== Compartir (iconos blancos, tamaño normal) ===== */
function svg(icon){
  const I={
    wa:'<svg viewBox="0 0 24 24"><path fill="currentColor" d="M20.5 3.5A11.5 11.5 0 1 0 3.5 20.5 11.5 11.5 0 0 0 20.5 3.5Zm-8.2 16.4a9 9 0 1 1 9-9 9 9 0 0 1-9 9Zm4.6-6.1c-.2-.1-1.3-.7-1.5-.8s-.4-.1-.6.1-.7.8-.8.9-.3.1-.5 0a6.9 6.9 0 0 1-2-1.3 7.5 7.5 0 0 1-1.4-1.7c-.1-.2 0-.4.1-.5l.4-.5a1.6 1.6 0 0 0 .2-.6.6.6 0 0 0 0-.6c0-.1-.6-1.5-.8-2s-.4-.5-.6-.5h-.5a1 1 0 0 0-.7.3 3 3 0 0 0-1 2.2 5.2 5.2 0 0 0 1.1 2.7 11.8 11.8 0 0 0 4.6 4.5 10.3 10.3 0 0 0 2.1.8 4.9 4.9 0 0 0 2.2.1 2.5 2.5 0 0 0 1.6-1.1 2 2 0 0 0 .1-1.1c-.1-.1-.3-.2-.5-.3Z"/></svg>',
...
function buildShareButtons(item){
  ...
  for(const b of list){
    const a=document.createElement('a'); a.className=`share-btn ${b.cls}`; a.innerHTML=`${svg(b.icon)}<span>${b.label}</span>`;
    if(b.action==='copy'){ a.href='#'; a.addEventListener('click',e=>{e.preventDefault(); navigator.clipboard?.writeText(`${text} ${url}`); alert('Enlace copiado. Pégalo en Instagram u otras apps.');});}
    else {a.href=b.href; a.target='_blank'; a.rel='noopener';}
    wrap.appendChild(a);
  }
  if(navigator.share){ const n=document.createElement('a'); n.className='share-btn'; n.innerHTML=`${svg('cp')}<span>Compartir…</span>`; n.href='#'; n.onclick=async e=>{e.preventDefault(); try{await navigator.share({title:'Riesgo de automatización',text,url});}catch{}}; wrap.prepend(n);}
}
...
/* ===== Audio: carga por BlobURL (silenciosa) ===== */
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  await cargarDatos();
  const input=$('#search');
  const paint=()=>{const l=sugerencias(input.value,10); const c=$('#sugerencias'); c.innerHTML=''; l.forEach(t=>{const b=document.createElement('button'); b.type='button'; b.textContent=t; b.onclick=()=>{input.value=t;c.innerHTML='';accionBuscar();}; c.appendChild(b);}); const dl=$('#opts'); if(dl){dl.innerHTML=''; l.forEach(t=>{const o=document.createElement('option'); o.value=t; dl.appendChild(o);});}};
  ...
  const params=new URLSearchParams(location.search); const q=params.get('q')||params.get('busqueda'); if(q){input.value=q; accionBuscar();}
  initAudio();
});
